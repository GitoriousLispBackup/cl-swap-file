...
(defstruct swap-file
  ...
  (block-cache    (make-hash-table :weakness :value) :type hash-table)               ; list of weak pointers pointing to a disk blocks.
...
(defun find-open-disk-block (swap-file offset)
  "Finds and returns already opened disk block with given offset."
  (find offset (swap-file-open-blocks swap-file) :key #'disk-block-offset))

(defun push-to-open (swap-file disk-block)
  (when disk-block
    (push disk-block (swap-file-open-blocks swap-file)))
  disk-block)

(defun remove-from-open (swap-file disk-block)
  (setf (swap-file-open-blocks swap-file) (remove disk-block (swap-file-open-blocks swap-file) :test #'eq)))
...
;;  (push (tg:make-weak-pointer disk-block) (swap-file-block-cache swap-file))
  (setf (gethash (disk-block-offset disk-block) (swap-file-block-cache swap-file)) disk-block))

(defun remove-from-cache (swap-file disk-block)
  (remhash (disk-block-offset disk-block) (swap-file-block-cache swap-file)))

(defun get-from-cache (swap-file offset)
  "Returns disk block from cache, or nil if disk block with given
offset does not exist."
  (gethash offset (swap-file-block-cache swap-file)))
;;   (do ((pos (swap-file-block-cache swap-file))
;;        (db nil))
;;       ((or db (null pos)) db)
;;     (let ((v (tg:weak-pointer-value (car pos))))
;;       (if v
;;          (progn
;;            (when (= (disk-block-offset v) offset)
;;              (setq db v))
;;            (setq pos (cdr pos)))
;;          (pop pos))))) ; remove broken weak pointer
...
(defun create-block (swap-file &key (next 0) (data (make-empty-data swap-file)))
  ...
  (let ((disk-block
    ...
    (push-to-open swap-file disk-block)
    (set-block-data disk-block data)))

(defun open-block (swap-file offset)
  "Opens and returns existing disk block from swap-file."
  (flet ((from-open-list ()
           (find-open-disk-block swap-file offset))
         (from-cache ()
           (push-to-open swap-file
                         (get-from-cache swap-file offset)))
         (from-disk ()
           (push-to-open swap-file
                         (push-to-cache swap-file (read-disk-block swap-file offset)))))
    (or (from-open-list)
        (from-cache)
        (from-disk))))
...
(defun close-block (swap-file disk-block)
  "Closes disk block."
  (write-disk-block disk-block swap-file)
  (remove-from-open swap-file disk-block))
...
(defun unlink-block (swap-file disk-block-spec &aux (disk-block (ensure-disk-block disk-block-spec swap-file)))
  ...
  (let ((next (disk-block-next disk-block)))
    ...
    (remove-from-open swap-file disk-block)
    (remove-from-cache swap-file disk-block)
